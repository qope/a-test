...
/// A method for deciding what arity to use at each reduction layer.各縮小層で使用するアリティを決定する方法。
#[derive(Debug, Clone, Eq, PartialEq)]
pub enum FriReductionStrategy {
    /// Specifies the exact sequence of arities (expressed in bits) to use.    /// 使用するアリティの正確なシーケンス（ビットで表現）を指定する。
    Fixed(Vec<usize>),
